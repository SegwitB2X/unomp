...
var generateOutputTransactions = function(poolRecipient, recipients, rpcData, premine){
    ...
    if (premine && premine.height === rpcData.height) {

        console.log("Premine:", premine);

        for (var i = 0; i < premine.addresses.length; i++) {

            var e = premine.addresses[i],
                script = util.addressToScript(e.address);

            txOutputBuffers.push(Buffer.concat([
                util.packInt64LE(e.amount),
...
exports.CreateGeneration = function(rpcData, publicKey, extraNoncePlaceholder, reward, txMessages, recipients, auxMerkleTree, premine){
    ...
    var outputTransactions = generateOutputTransactions(publicKey, recipients, rpcData, premine);
